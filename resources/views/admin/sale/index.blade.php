...
document.addEventListener('DOMContentLoaded', function () {
    const tableIdToExport = 'salesTable';
        ...
        try {
            dataTableInstance = $(`#${tableIdToExport}`).DataTable({
                ...
                "columnDefs": [
                    ...
                    { "type": "date-eu", "targets": 1 }, // Fecha (formato dd/mm/yyyy H:i)
    ...
    // --- Exportación a PDF ---
    function exportListToPdf(filename = 'listado_ventas.pdf') {
        ...
        try {
            ...
            console.error("Error al generar PDF del listado de ventas:", error);
            alert("Error al generar PDF del listado de ventas. Verifique la consola para más detalles.");
        }
    }
    ...
    function parseNumericValue(text) {
        ...
        let cleanText = String(text).trim().replace(/^(S\/\s*|\$\s*|€\s*)/, ''); // Quitar S/ u otros símbolos
            ...
            for (let C = range.s.c; C <= range.e.c; ++C) {
                ...
                if (ws[cell_ref].t === 'n') { // Si es número
                    if (headerName === 'id') {
                        ...
                        ws[cell_ref].s.numFmt = '"S/" #,##0.00'; // Moneda S/
    ...
    document.getElementById('confirmExcelExportBtn')?.addEventListener('click', () => {
        if (excelFilenameInput && dataTableInstance) {
            ...
            exportDataToExcel(filename, 'Listado Ventas', dataTableInstance);
